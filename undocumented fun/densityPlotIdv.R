densityPlotIdv <- function(data,level, unit, x,title = NULL, xlab = NULL, caption = NULL, legendTitle = NULL, legendLabel = NULL) {
  #` Plot the probability density in individual level with the level(e.g., geography type) mean as vline
  ...
  gg <- ggplot(df, aes(x, fill = level, colour = level)) +
