...
RidgeDensityPsu <- function(data, dfBoxplot, dv, xlabel = "Proportion of Sampling Unit Who Used a Toilet For Last Defecation", captiontext = "Kernel density estimates and box plots with individual data points of sampling unit level prevalence \nof toilet use for last defecation. Data disaggregated by geography type.", color = mycolors, adjust = 0.5) {
  ...
  # gray scale suggestion red-blue-gray
  mygrays <- c("#ffffff", "#808080", "#000000")
  ...
  g <- ggplot(data = w2, aes(y = voi, x = psutype, fill = psutype)) +
    ...
    geom_boxplot(inherit.aes = F,data = dfBoxplot, aes(x = psutype, fill = psutype, ymin = value0, lower = value25, middle = value50, upper = value75, ymax = value100), stat = "identity", width = .1, outlier.shape = NA, alpha = 0.1, colour = "black") +
    geom_point(aes(y = voi, color = psutype), position = position_jitter(width = .00, height = 0.00), size = 3, shape = 108, alpha = 1, colour = "black") +
